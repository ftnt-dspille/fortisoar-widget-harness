...
function lintWidget(opts) {
  ...
  const known = new Set([...ANGULAR_BUILTINS, ...registeredServices]);
  const checkDeps = (file) => {
    ...
    if (unknown.length > 0) {
      errors.push({
        ...
        file,
        message: `${file} injects services not registered on the harness: ${unknown.join(", ")}. Add a stub to harness.module.js.`,
        unknown,
      });
    }
  };
